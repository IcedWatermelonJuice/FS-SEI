...
def train_and_test(encoder, classifier, loss_nll, train_dataloader, val_dataloader, optim_encoder, optim_classifier,
    ...
    for epoch in range(1, epochs + 1):
        train(encoder, classifier, loss_nll, train_dataloader, optim_encoder, optim_classifier, epoch, device, writer, freeze_encoder)
...
def run(train_dataloader, val_dataloader, test_dataloader, epochs, device, writer, encoder_name, freeze_encoder, is_pretrained, pretrained_path, **opt_params):
...
def main():
    ...
    for i in range(config['iteration']):
        ...
        test_acc = run(train_dataloader, val_dataloader, test_dataloader, epochs=params['max_epochs'], device=device,
                       writer=writer, encoder_name=params['encoder'], freeze_encoder=params['freeze'], is_pretrained=params["pretrain"],
