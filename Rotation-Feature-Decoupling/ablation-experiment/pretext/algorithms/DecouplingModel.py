...
from utils import signalRotation
...
class DecouplingModel(Algorithm):
    ...
    def train_step(self, batch):
        ...
        dataX_90=signalRotation(dataX,90)
        dataX_180=signalRotation(dataX,180)
        dataX_270=signalRotation(dataX,270)

        dataX = torch.stack([dataX, dataX_90, dataX_180, dataX_270], dim=1)
        batch_size, rotations, channels, length= dataX.size()
        dataX = dataX.view([batch_size*rotations, channels, length])
        ...
        with torch.set_grad_enabled(True):
            ...
            feature_invariance_instance = torch.mul(feature_invariance_instance, 0.25)  # size 32，2048
        ...
        with torch.set_grad_enabled(True):
            ...
            if self.ablated_loss != "loss_cls":
                loss_total = self.lambda_loss['mse']*loss_mse + self.lambda_loss['nce']*loss_nce
            elif self.ablated_loss != "loss_mse":
                loss_total = self.lambda_loss['cls'] * loss_cls + self.lambda_loss['nce'] * loss_nce
            elif self.ablated_loss != "loss_nce":
