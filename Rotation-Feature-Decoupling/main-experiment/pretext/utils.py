...
def signalRotation(iq_matrix, angle):
    radians = np.radians(angle)
    cos = np.cos(radians)
    sin = np.sin(radians)
    rotation_matrix = torch.tensor([[cos, -sin], [sin, cos]],dtype=torch.float,device="cuda")
    rotated_iq_matrix = torch.matmul(rotation_matrix, iq_matrix)
    return rotated_iq_matrix
